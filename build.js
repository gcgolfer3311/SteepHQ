...
const SITE = "https://steepteahq.com";
const SITE_NAME = "SteepHQ";

function loadData(name) {
  const filePath = `data/${name}.json`;
  if (!fs.existsSync(filePath)) {
    console.error(`\nBUILD FAILED: missing ${filePath}`);
    console.error(`This file must exist in the data/ folder of the repo. Check that it was actually uploaded to GitHub (not just present locally).\n`);
    process.exit(1);
  }
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf8'));
  } catch (e) {
    console.error(`\nBUILD FAILED: ${filePath} exists but isn't valid JSON.`);
    console.error(`Parse error: ${e.message}`);
    console.error(`This usually means the file is empty, or content got cut off during upload/paste.\n`);
    process.exit(1);
  }
}
const regions = loadData('regions');
const teas = loadData('teas');
...
<!-- Google Analytics (GA4) — replace G-980V4Z186G with your real Measurement ID -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-980V4Z186G"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());
  gtag('config', 'G-980V4Z186G');
</script>
